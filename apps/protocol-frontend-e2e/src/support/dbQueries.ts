...
export const create_GuildContribution = (
  ObjectData: GuildContributionObjectType,
) => {
  const query = `
...
export const create_GuildUser = (GuildUserObject: GuildUserObjectType) => {
  const query = `
    INSERT INTO "GuildUser" (user_id, guild_id, membership_status_id)
    VALUES (${GuildUserObject.userID}, ${GuildUserObject.guildID}, ${GuildUserObject.membershipID});
  `;
  return queryDB(query);
};
...
export const delete_GuildContribution = (guildID: number) => {
  const query = `
    DELETE
    FROM "GuildContribution" CASCADE
    WHERE guild_id = '${guildID}';
  `;
  return queryDB(query);
};

export const delete_MembershipStatus = () => {
  const query = `DELETE
...
export const create_MembershipStatus = () => {
  const query = `
