...
interface DeleteContributionProps {
...
export const useContributionDelete = () => {
  ...
  const { mutate, mutateAsync, isLoading, isError, isSuccess } = useMutation(async ({ updatedValues, contribution }: DeleteContributionProps) => {
    if (userData !== null) {
